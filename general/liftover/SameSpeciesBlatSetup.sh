...
export workDirectory="/share/adl/ytao7/summer/liftover/SameSpeciesTry1"
...
export twoBitDirectory="/share/adl/ytao7/summer/liftover/SameSpeciesTry/2bit"

#######################################################################
# targetDb is the sequence you have good coordinates you want to convert
export targetDb="initial_assembly"
# typical chunk size for a target sequence is 10,000,000 bases
export targetChunkSize="25000000"
...
/data/users/ytao7/software/kentUtils/src/hg/utils/automation/partitionSequence.pl ${targetChunkSize} \
...
# check the usage message for this command in the kent source tree
#   to see what all the arguments mean: ${queryChunkSize} \
/data/users/ytao7/software/kentUtils/src/hg/utils/automation/partitionSequence.pl ${queryChunkSize} \
...
export realSize=`basename $realSize`
...
blat ${twoBitDirectory}/${targetDb}.2bit /dev/null \
        /dev/null -tileSize=11 -makeOoc=11.ooc -repMatch=875
...
cat << EOF > template
#LOOP
./BlatJob.csh ${workDirectory}/run.blat/\$(path1) ${workDirectory}/run.blat/\$(path2) ${workDirectory}/run.blat/psl/\$(file1)/\$(file2).psl
#ENDLOOP
EOF
