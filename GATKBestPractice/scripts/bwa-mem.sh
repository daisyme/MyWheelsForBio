...
bwa mem -t $CORES -R '@RG\tID:'$samplename'\tPU:'$barcode.$lane.$samplename'\tSM:'$samplename'\tPL:illumina\tLB:'$library -M $ref $idir/${prefix}READ1-Sequences.txt.gz $idir/${prefix}READ2-Sequences.txt.gz | samtools view -bS - > $odir/${prefix}$samplename.bam 
