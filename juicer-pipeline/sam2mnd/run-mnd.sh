...
workDir="/the/path/to/where/you/want/to/work"
...
do
    ...
    awk -v "fname1"=${name}${ext}_norm.txt -v "fname2"=${name}${ext}_abnorm.sam -v "fname3"=${name}${ext}_unmapped.sam -f ${workDir}/scripts/chimeric_blacklist.awk $name$ext.sam
   ...
   awk -f ${workDir}/scripts/dups.awk -v name="${outputdir}/${library}_" ${outputdir}/${library}_merged_sort.txt
done
...
awk -f ${workDir}/scripts/dups.awk -v name=${outputdir}/ ${outputdir}/merged_sort.txt
