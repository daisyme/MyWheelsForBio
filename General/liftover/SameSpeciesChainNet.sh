...
cat << EOF > template
#LOOP
chainJob.sh \$(path1) chainRaw/\$(path1).chain
#ENDLOOP
EOF
...
targetDb="${targetDb}"
queryDb="${queryDb}"
tmpOut=\`mktemp -p /dev/shm doSame.chain.XXXXXX\`
...
gensub2 pslParts.lst single template jobList_chain
